...
import { mockBatchInvoiceItems } from "../data/batch-invoice.mock";
import {
  getInvoiceTotal,
  getUtilitySubtotal,
...
export const BatchInvoicePage = () => {
  const form = useForm<BatchInvoiceValues>({
    ...
    defaultValues: {
      selectedInvoiceIds: mockBatchInvoiceItems.map((item) => item.id),
    },
  });
  ...
  const { isAllSelected, toggleSelect, toggleSelectAll } =
    useBatchInvoiceSelection(
      selectedInvoices,
      mockBatchInvoiceItems.map((item) => item.id),
